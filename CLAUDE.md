...
Visual system helpers in `globals.css`: `.on-bone` (sand section), `.on-ink` (dark section), `.dots-bg` / `.rings-bg` (faint plum textures), `[data-reveal]` (scroll-in via `components/Reveal.tsx`). Don't claim licences we don't hold (the wireframe's "MGA" was dropped) and never publish invented testimonials.

## Design language

**"White" redesign (Sep 2026, branch `redesign/1-white`)** — the template came from Claude Design (`revolver ui white.zip`, homepage only); every other page follows the same system. Warm paper ground `#faf8f4`, sand sections `#f0ebe4`, ink `#261028` for text and the final dark chambers (exclusives banner, contact, footer), plum `#5a2a60` for italic accents and the stat block, amber `#ff9a3d` **only** for real CTAs. Squared edges everywhere, no border-radius, no shadows beyond a faint hover lift, hairlines at `rgba(38,16,40,.14)`. Fonts via `next/font`: Archivo (display, 700, tight tracking, italic 600 for the accented word) and Figtree (body); the admin keeps IBM Plex Mono for itself.

Colour is driven by **context tokens** in `globals.css`: `:root` paints the paper context, `.on-bone` flips to sand, `.on-ink` flips to dark. Components only read the semantic tokens (`--bg`, `--card`, `--fg`, `--fg-muted`, `--fg-soft`, `--em`, `--line`, `--ghost`) so one rule set serves every section — add new components the same way rather than writing `.on-bone .x` overrides.

The hero visual is the **deck** (`components/HeroDeck.tsx`): the featured games' banners at their native 680×440 ratio, one in front with the next three queued beneath, auto-advancing. (The earlier revolver-cylinder carousel was dropped: cropping the banner art into circular chambers didn't suit the thumbnails, and Ryan didn't like it.) The revolver metaphor runs through the copy — "Our arsenal", "Four chambers, one platform", "Last chamber" for CTAs. New sections should extend this metaphor and tone, not fight it.

Brand assets (logos, emblem) are in `public/brand/` — the purple wordmark on paper, the white one on ink; game art is served from Supabase storage. The admin CMS has its own stylesheet (`app/admin/admin.css`) with a self-contained dark palette.
