...
Brand assets (logos, emblem) are in `public/brand/`; game art is mostly served from Supabase storage, with wide banners in `public/banners/` and a few cards in `public/cards/`. An `AgeGate` (18+, answer kept in `localStorage`) blocks the site on first visit from `app/(site)/layout.tsx`.
