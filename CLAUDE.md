...
Both game detail pages (`/game/{slug}`, `/originals/{slug}`) share one `.game-body` grid: `.game-intro` (eyebrow/title/blurb/CTAs) and `.game-copy` (description or feature points) stack in the left column, `.game-panel` spans both rows on the right and is a pure spec sheet. Ryan wants the CTAs beside the copy in `.game-ctas`: Play demo in amber, everything else as ghost buttons, and a `.btn-inert` "Demo soon" placeholder when `demo_url` is empty. The demo overlay must stay a portal to `<body>` — see the `DemoLauncher` comment; `.game-panel` is `position: sticky`, which creates a stacking context and would otherwise bury the open game behind the nav and the cards below it.
...
Brand assets (logos, emblem) are in `public/brand/`; game art is mostly served from Supabase storage, with wide banners in `public/banners/` and a few cards in `public/cards/`. `app/(site)/layout.tsx` mounts three first-visit pieces: `AgeGate` (18+, blocking), `CookieConsent` (a corner card, shown only once the age check clears) and `Analytics`. Their stored answers live in `lib/consent.ts`. GA4 loads **only** on accept and needs `NEXT_PUBLIC_GA_ID` (a `G-…` id) — the old site's `UA-…` property no longer collects data and cannot be reused.
