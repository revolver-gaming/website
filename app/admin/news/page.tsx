...
export default function AdminNews() {
    ...
    const { rows, count, page, setPage, error } =
        usePaged<Row>("news", "id, slug, title, published_at, published", "published_at", false, q);

    return (
        <section>
            <div className="admin-head">
                <h1 className="display">News</h1>
                <Link className="btn btn-fire" href="/admin/news/new">+ New article</Link>
            </div>
            {error && <p className="admin-error">{error}</p>}
            <div className="admin-toolbar">
                <input className="admin-search" placeholder="Search by title…" value={q} onChange={(e) => setQ(e.target.value)} />
                <span className="mono">{count} article{count === 1 ? "" : "s"}</span>
            </div>
                    ...
                    {rows?.map((r) => (
                        <tr key={r.id}>
                            ...
                            <td className="admin-row-actions">
                                {r.published && <a href={`/news/${r.slug}`} target="_blank">View ↗</a>}
                                <Link href={`/admin/news/${r.id}`}>Edit</Link>
                            </td>
                        </tr>
                    ))}
                    {rows?.length === 0 && (
                        <tr><td colSpan={4} className="admin-empty">{q ? "No matches." : "No articles yet."}</td></tr>
                    )}
                </tbody>
            </table>
